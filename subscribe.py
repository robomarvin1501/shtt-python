...
def keep_alive(shutdown: queue.Queue, use_tls, addr, username, password, reminder=10):
    while True:
...
def subscriber(use_tls, channel, username, password, broker_addr=("localhost", SHTT.PORT),
               subscriber_addr=("localhost", SHTT.PORT)):
    ...
    subscription_message.username = username
    subscription_message.password = password
    SHTT.send_message(subscription_message.encode(), use_tls, broker_addr)

    shutdown = queue.Queue()

    thread_keep_alive = threading.Thread(
        target=keep_alive, args=(shutdown, use_tls, broker_addr, username, password))
...
if __name__ == "__main__":
    ...
    subscriber(args.tls, args.channel, args.username, args.password, (args.broker, args.port),
